...
// Seed network for the 2026 Poson plan, Anuradhapura.
// Coordinates are approximate, traceable positions for the corridor running
// Pothanegama -> Yapanaya (~13 km) past the Atamasthana sacred sites.
// Police staff refine the exact polylines in the Map Editor.
//
// NOTE: this is demonstration seed data so the app works out of the box.
// It is NOT the legally binding plan — verify against the official arrow maps.

const N = (id, name_si, name_en, lat, lng, extra = {}) => ({
  id,
  name_si,
  name_en,
  lat,
...
export const seedNodes = [
  N("pothanegama", "පොතානේගම හන්දිය", "Pothanegama Junction", 8.3210, 80.4060, { isEntryPoint: true }),
  N("mirisaveti", "මිරිසවැටිය", "Mirisaveti", 8.3447, 80.3930),
  ...
  N("thuparamaya", "ථූපාරාමය", "Thuparamaya", 8.3536, 80.3960),
  N("lankarama", "ලංකාරාමය", "Lankarama", 8.3585, 80.3905),
  N("abhayagiri", "අභයගිරිය", "Abhayagiri", 8.3636, 80.3958),
  N("jetavanaramaya", "ජේතවනාරාමය", "Jetavanaramaya", 8.3500, 80.4030),
  N("sacredcityjn", "පූජා නගර හන්දිය", "Sacred City Junction", 8.3420, 80.4010),
  N("yapanaya", "යාපනය හන්දිය", "Yapanaya Junction", 8.3700, 80.4040, { isExitPoint: true }),
  N("busstandjn", "බස් නැවතුම් හන්දිය", "Bus Stand Junction", 8.3360, 80.4090, { isExitPoint: true }),
];
...
const S = (id, from, to, name_si, name_en, notes = "") => {
  const polyline = line(from, to);
  return {
    id,
    fromNodeId: from,
    toNodeId: to,
...
// Directed one-way corridor. Forms a navigable loop so routing is meaningful:
// inbound spine south->north along the sacred sites, outbound spine back east.
export const seedSegments = [
  ...
  // outbound return spine (east side), one-way the other way
  ...
  // a short connector giving an alternate legal path
  S("s13", "thuparamaya", "jetavanaramaya", "සම්බන්ධක මාර්ගය", "Connector Rd"),
];

export const seedDansal = [
  { id: "d1", name_si: "බෝධි දන්සැල", name_en: "Bodhi Dansala", lat: 8.3470, lng: 80.3958, type: "food", nearestSegmentId: "s4", active: true, openHours: "06:00–22:00" },
  { id: "d2", name_si: "රුවන්වැලි පැන් දන්සැල", name_en: "Ruwanweli Drinks", lat: 8.3496, lng: 80.3970, type: "drink", nearestSegmentId: "s6", active: true, openHours: "06:00–24:00" },
  { id: "d3", name_si: "ථූපාරාම ජල දන්සැල", name_en: "Thuparama Water", lat: 8.3540, lng: 80.3955, type: "water", nearestSegmentId: "s7", active: true, openHours: "24h" },
  { id: "d4", name_si: "අභයගිරි වෛද්‍ය මධ්‍යස්ථානය", name_en: "Abhayagiri Medical", lat: 8.3640, lng: 80.3962, type: "medical", nearestSegmentId: "s8", active: true, openHours: "24h" },
